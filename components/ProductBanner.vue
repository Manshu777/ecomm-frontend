  ...
  <div class="white-cards">
    <div class="white-card" v-for="(card, index) in cards" :key="index">
      <div class="category_titles">
           <h5>Women's Trendy Bags | Up to 55% off</h5>
         </div>
      <div class="card-row">
...
export default {
  ...
  async mounted() {
    try {
      const response = await this.$axios.$get('https://admin.swascabs.com/api/v1/all?category=womens-bags');
      ...
      // Group limited products into cards
      let cards = [];
      for (let i = 0; i < limitedProducts.length; i += 4) {
        cards.push({ products: limitedProducts.slice(i, i + 4) });
      }
      this.cards = cards;
